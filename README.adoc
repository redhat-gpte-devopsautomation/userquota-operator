...
This operator requires cluster administrator permissions. Either use `system:admin` or a user id that has been granted the `cluster-admin` role. 
...
. Deploy the *sonarqube-operator* service account (change the project name if you deployed into a different project):
+
[source,sh]
----
